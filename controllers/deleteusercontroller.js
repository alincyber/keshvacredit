...
async function resolveUser(phone, email) {
  ...
  let user = await PersonalUser.findOne({
    person_phone: phone,
    person_email: email,
  });
  ...
  try {
    ...
    (async () => {
      try {
        ...
        userObj.deleteAt = user.deleteAt.toLocaleString("en-IN", {
          timeZone: "Asia/Kolkata",
        });

        const fieldMap =
          source === "personal" ? personalFieldMap : formFieldMap;
        const recipientName =
          source === "personal"
        ...
        let pdfFilename;
        try {
          pdfBuf = await generatePDF("Account Deletion", userObj, fieldMap);
          pdfFilename = `AccountDeletion_${user._id}.pdf`;
        } catch (pdfErr) {
          logger.warn(
            { err: pdfErr, id: user._id },
            "PDF generation failed for deletion email, sending text-only email",
          );
        }
        ...
        logger.error(
          ...
          "Failed to send deletion PDF email",
        );
      }
    })();
    ...
    return res.status(200).json({
      success: true,
      message:
        "Account deletion scheduled successfully. Your account will be deleted after 48 hours.",
